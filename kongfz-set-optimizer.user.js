  ...
  function buildPanel() {
    GM_addStyle(`
      ...
      #kfz-result .kfz-ex{display:inline-block;margin:0 0 0 6px;padding:0 5px;line-height:16px;
        background:#f6f6f6;color:#999;border:1px solid #e2e2e2;border-radius:4px;
        font-size:11px;font-weight:400;cursor:pointer;vertical-align:middle}
      #kfz-result .kfz-ex:hover{background:#c8161d;color:#fff;border-color:#c8161d}
      /* 底部折叠区：已排除条目 */
  ...
  async function runWith(data, logf, result, isDemo) {
    ...
    if (!res.ok) {
      ...
      let eh = '<div class="kfz-err">❌ ' + escapeHtml(res.error || '无法凑齐') +
        (blocked ? '（当前因排除少算 ' + blocked + ' 条，可在下方折叠区减少规则或恢复条目）' : '') + '</div>';
